...
public enum AsyncOperationActionType implements com.oracle.bmc.http.internal.BmcEnum {
    Unknown("UNKNOWN"),
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(AsyncOperationActionType.class);

    private final String value;
    private static java.util.Map<String, AsyncOperationActionType> map;

    static {
        map = new java.util.HashMap<>();
        for (AsyncOperationActionType v : AsyncOperationActionType.values()) {
    ...
    AsyncOperationActionType(String value) {
        this.value = value;
    }
    ...
    public static AsyncOperationActionType create(String key) {
        if (map.containsKey(key)) {
            return map.get(key);
        }
        LOG.warn("Received unknown value '{}' for enum 'AsyncOperationActionType', returning UnknownEnumValue", key);
