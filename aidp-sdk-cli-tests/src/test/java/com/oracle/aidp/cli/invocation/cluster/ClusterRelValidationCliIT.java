...
public class ClusterRelValidationCliIT {
  ...
  private static final String METRIC_CPU_UTILIZATION = "CPU_UTILIZATION";
  private static final String AGGREGATION_INTERVAL = "1m";
  private static final String METRIC_RESOLUTION = "1m";
            ...
            () ->
                runner.run(
                    invocationBuilder.commandWithBody(
                        ...
                        workspaceKey,
                        clusterKey)),
            node -> hasNamedArray(data(node), "results", "aggregatedDataPoints"));
  ...
  private boolean hasNamedArray(JsonNode node, String... fieldNames) {
    if (node == null || node.isMissingNode() || node.isNull()) {
      return false;
    }
    if (node.isObject()) {
      ...
      while (fields.hasNext()) {
        Map.Entry<String, JsonNode> field = fields.next();
        if (matchesAny(field.getKey(), fieldNames) && field.getValue().isArray()) {
          return true;
        }
        if (hasNamedArray(field.getValue(), fieldNames)) {
          return true;
        }
      }
    }
    if (node.isArray()) {
      for (JsonNode item : node) {
        if (hasNamedArray(item, fieldNames)) {
          return true;
        }
      }
    }
  ...
  private String metricsBody() {
    return """
        ...
        """
        .formatted(
            config.value("summarizeMetricsMetricName", METRIC_CPU_UTILIZATION),
            config.value("summarizeMetricsAggregationType", "MEAN"),
            config.value("summarizeMetricsInterval", AGGREGATION_INTERVAL),
            config.value("summarizeMetricsResolution", METRIC_RESOLUTION),
            observabilityWindowBegin(),
            observabilityWindowEnd());
  }
